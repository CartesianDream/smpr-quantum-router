...
FROZEN_CROSS_LAYOUT_LAYOUTS = 0
FROZEN_CROSS_LAYOUT_NEIGHBORS = 4
FROZEN_PROXY_DECAY = 0.95

PRIMARY_QUALITY_COLUMNS = (
    "lightsabre_swap",
    "independent_rollout_swap",
    "baseline_rollout_backend",
...
AUXILIARY_QUALITY_COLUMNS = (
    "lightsabre_depth",
    "independent_rollout_depth",
    "baseline_rollout_safe_depth",
    "cross_layout_depth",
    "cross_layout_attempts",
)

QUALITY_COLUMNS = PRIMARY_QUALITY_COLUMNS + AUXILIARY_QUALITY_COLUMNS

REFERENCE_TIMING_RE = re.compile(
...
def parse_reference_timings(path: Path | None) -> dict[int, float]:
    if path is None:
        return {}
    timings: dict[int, float] = {}
    text = decode_console_bytes(path.read_bytes())
    for line in text.splitlines():
        match = REFERENCE_TIMING_RE.search(line.strip())
        if match is None:
            continue
    ...
    if not timings:
        raise ValueError(f"no historical development case timings found in: {path}")
    return timings
...
def validate_shard_output(task: ShardTask, selection_mode: str) -> bool:
    ...
    return (
        ...
        and str(summary.get("selection_mode", "legacy")) == selection_mode
    )


def run_shard(
    task: ShardTask,
    portfolio_script: Path,
    input_path: Path,
    selection_mode: str,
    resume: bool,
) -> ShardResult:
    if resume and validate_shard_output(task, selection_mode):
        return ShardResult(
    ...
    command = [
        ...
        str(FROZEN_CROSS_LAYOUT_LAYOUTS),
        "--cross-layout-neighbors",
        str(FROZEN_CROSS_LAYOUT_NEIGHBORS),
        "--proxy-decay",
...
def compare_quality(
    ...
    reference_path: Path,
    columns: tuple[str, ...] = QUALITY_COLUMNS,
) -> list[dict[str, str]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "persistent runner: exact persistent-shard executor for frozen SMPR portfolio"
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--timing-reference",
        type=Path,
        help="historical development console log used only for LPT load balancing",
    )
    parser.add_argument(
        "--portfolio-script",
        type=Path,
        default=Path(__file__).resolve().with_name(
            "smpr_portfolio_cross_layout_safe_cross_layout.py"
        ),
    )
    ...
    parser.add_argument(
        ...
        default="exact",
        help=(
            "exact compares all frozen quality fields; primary excludes "
            "auxiliary candidate depths/attempt counts handled by historical development"
        ),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/persistent_runner"),
    )
    parser.add_argument(
        "--selection-mode",
    ...
    if not args.portfolio_script.is_file():
        raise FileNotFoundError(args.portfolio_script)
    if (
        args.quality_reference is not None
        and not args.quality_reference.is_file()
    ):
    ...
    measured_timings = parse_reference_timings(args.timing_reference)
    ...
    print("===== SMPR persistent-shard execution =====")
    ...
    print("SMPR portfolio:", args.portfolio_script.resolve())
    print(
        "frozen parameters:",
        f"LS={FROZEN_QISKIT_SEEDS}x{FROZEN_QISKIT_REPEATS}, "
        f"cross-layout bases={FROZEN_CROSS_LAYOUT_LAYOUTS}, "
        f"neighbors/base={FROZEN_CROSS_LAYOUT_NEIGHBORS}, "
        f"decay={FROZEN_PROXY_DECAY}",
    )
    print("quality reference:", args.quality_reference)
    print("quality policy:", args.quality_policy)
    ...
    if failed:
        ...
        for result in failed:
            ...
            print("\n".join(lines[-40:]))
        raise RuntimeError(f"{len(failed)} SMPR portfolio shard processes failed")
    ...
    for task in tasks:
        if not validate_shard_output(task, args.selection_mode):
    ...
    if args.quality_reference is not None:
        quality_columns = (
            QUALITY_COLUMNS
            if args.quality_policy == "exact"
            else PRIMARY_QUALITY_COLUMNS
        )
        mismatches = compare_quality(
            case_rows,
    ...
    independent_total = sum(int(row["independent_rollout_swap"]) for row in case_rows)
    dual_total = sum(int(row["baseline_rollout_safe_swap"]) for row in case_rows)
    ...
    summary = {
        "schema_version": 1,
        "component": "persistent_runner",
        "purpose": "exact persistent-shard execution of frozen SMPR portfolio",
        "input": str(args.input.resolve()),
        "input_sha256": sha256(args.input),
        "portfolio_script": str(args.portfolio_script.resolve()),
        "smpr_portfolio_sha256": sha256(args.portfolio_script),
        ...
        "frozen_parameters": {
            ...
            "cross_layout_layouts": FROZEN_CROSS_LAYOUT_LAYOUTS,
            "cross_layout_neighbors": FROZEN_CROSS_LAYOUT_NEIGHBORS,
            "proxy_decay": FROZEN_PROXY_DECAY,
        },
        "totals": {
            "lightsabre_swap": ls_total,
            "independent_rollout_swap": independent_total,
        ...
        "selected_branches": dict(
            Counter(
                ...
                }.get(row["selected_backend"], row["selected_backend"])
                for row in case_rows
            )
        ),
    ...
    print("independent rollout total SWAP:", independent_total)
    print("baseline/rollout safe total SWAP:", dual_total)
    print("SMPR total SWAP:", selected_total)
