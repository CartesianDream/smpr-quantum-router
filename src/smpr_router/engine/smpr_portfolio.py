...
import lightsabre_backend as lightsabre_backend
import independent_backend as independent_backend
import semantic_audit as semantic_audit
...
PUBLIC_BRANCH = {
    ...
    "independent_rollout": "independent_rollout",
...
def candidate_key(candidate: RouteCandidate) -> tuple:
    # 完全同质量时优先保留 LightSABRE：它有原生 QPY。
    backend_priority = {
        "lightsabre": 0,
        "cross_layout": 1,
...
def top_k_for_case(case: DevCase) -> int:
    return (
        4
        if routing_rules.frozen_variant(case)
        == "grid_alternating_policy_grid_n10_alternating_k4"
        else 2
    )


def run_independent_rollout_mapping(
    case: DevCase,
    mapping: tuple[int, ...],
    label: str,
    backend: str,
    ...
    # 冻结的 historical development/26 原生路径会先在同一个 HardwareGraph 上运行
    # 一次基线策略，再执行 rollout。除了计入原始预算，这还会以完全
    # 确定的方式预热最短路缓存。原生 independent rollout 必须保留此行为，才能与
    # 1843 的冻结基线逐例一致。交叉布局候选是独立的新后端，冷启动。
    if warm_base:
        base_state = rollout_policy.structural_policy.run_router(
            dag=dag,
            hardware=hardware,
    ...
    state, stats = rollout_policy.run_rollout_router(
        dag=dag,
        hardware=hardware,
        initial_mapping=list(mapping),
        base_config=config,
    ...
    return RouteCandidate(
        backend=backend,
        label=label,
        swap=rollout_policy.count_swaps(state),
...
def adaptive_mapping_mappings(case: DevCase) -> tuple[list[MappingCandidate], int, str]:
    mapping_mode, top_k = adaptive_mapping.budget_for_case(
        case=case,
        pool_modes=frozen_policy.POOL_MODES,
        default_mapping=frozen_policy.DEFAULT_MAPPING,
        default_top_k=frozen_policy.DEFAULT_TOP_K,
        pool_top_k=frozen_policy.POOL_TOP_K,
    )
    return (
        mapping_portfolio.make_mappings(case, mapping_mode),
        top_k,
        f"{mapping_mode}/k{top_k}",
    )


def local_layout_search_mappings(
    case: DevCase,
    config,
) -> tuple[list[MappingCandidate], int, str]:
    rows, candidates = local_layout_search.search_mapping_candidates(
        case,
        config,
    ...
    if not valid:
        raise RuntimeError(f"{case.name} 没有合法 historical development 映射")
    ...
    for row in ranked:
        if len(selected_names) >= frozen_policy.ROLLOUT_TOP:
            break
    ...
    return (
        ...
        frozen_policy.DEFAULT_TOP_K,
        "local-search-b2-r1/rollout2-k2",
    )


def layout_proxy_mappings(
    case: DevCase,
    config,
    top_k: int,
) -> tuple[list[MappingCandidate], int, str]:
    rows, candidates = layout_proxy.build_proxy_shortlist(
        case=case,
        config=config,
        proxy_decay=routing_rules.PROXY_DECAY,
        proxy_top=routing_rules.PROXY_TOP,
    ...
    if old_pool_name is None:
        raise RuntimeError(f"{case.name} 的旧映射不在 historical development 候选池")
    ...
    for row in ranked:
        if len(selected_names) >= routing_rules.ROLLOUT_TOP:
            break
        if row.mapping_name not in selected_names:
            selected_names.append(row.mapping_name)
...
def run_native_independent_rollout(
    case: DevCase,
    config,
    ...
    if variant == "local_layout_search_ring_n10_far":
        mappings, top_k, budget = local_layout_search_mappings(case, config)
    elif variant in {
        "layout_proxy_grid_uniform_k2",
        "grid_alternating_policy_grid_n10_alternating_k4",
    }:
        mappings, top_k, budget = layout_proxy_mappings(case, config, top_k)
    else:
        mappings, top_k, budget = adaptive_mapping_mappings(case)

    search_ms = (time.perf_counter() - search_start) * 1000.0
    candidates = [
        run_independent_rollout_mapping(
            case=case,
            mapping=tuple(item.mapping),
            label=f"native:{item.name}",
            backend="independent_rollout",
            config=config,
            top_k=top_k,
            warm_base=True,
        )
    ...
    # 映射池/代理搜索也属于原生 independent rollout 的在线预算。
...
def extract_qiskit_mappings(
    ...
    initial = semantic_audit.mapping_from_layout(
        routed.layout,
        "initial_virtual_layout",
        logical_qubits,
        routed.num_qubits,
    )
    final = semantic_audit.mapping_from_layout(
        routed.layout,
...
def run_lightsabre_trials(
    ...
    for seed in range(seeds):
        ...
        lightsabre_backend.validate_topology(routed, case.test_case.edges)
        candidates.append(
            RouteCandidate(
                backend="lightsabre",
...
def best_quality_layouts(
    ...
    ranked = sorted(
        ...
        key=(
            (lambda item: (
                int(item.seed if item.seed is not None else 2**31 - 1),
                tuple(item.initial_mapping),
                item.label,
            ))
            if SELECTION_MODE == "deterministic"
            else (lambda item: (item.runtime_ms, int(item.seed or 0)))
        ),
    )
...
def run_cross_layout_candidates(
    case: DevCase,
    qiskit_candidates: list[RouteCandidate],
    config,
    maximum_layouts: int,
        ...
        for label, mapping, score in proposed:
            ...
            output.append(
                run_independent_rollout_mapping(
                    case=case,
                    mapping=mapping,
                    label=f"cross_layout:{label}",
                    backend="cross_layout",
                    config=config,
                    top_k=top_k,
                    proxy_score=score,
                )
...
def export_and_audit(
    ...
    if selected.backend == "lightsabre":
        ...
        if selected.state is None:
            raise RuntimeError("independent rollout 候选缺少 RoutingState")
        _dag, hardware, _ = case.test_case.build()
        logical_cx, swap, expanded_cx = independent_backend.export_qasm(
            selected.state,
            hardware,
            qasm_path,
        )
        if logical_cx != case.two_qubit_gate_count or swap != selected.swap:
            raise AssertionError("independent rollout 导出计数不一致")
        if expanded_cx != logical_cx + 3 * swap:
            raise AssertionError("independent rollout SWAP 展开计数不一致")
    ...
    semantic_audit.validate_mapping(
    ...
    expected = semantic_audit.expected_physical_circuit(
        source=source,
        initial_mapping=list(selected.initial_mapping),
        final_mapping=list(selected.final_mapping),
    ...
    topology_error_list = semantic_audit.topology_errors(
        parsed,
        [list(edge) for edge in case.test_case.edges],
    )
    topology_valid = not topology_error_list
    actual_depth = semantic_audit.weighted_depth(parsed)
...
def summary_payload(
    ...
    return {
        ...
        "cross_layout_enabled": True,
        "cross_layout_layouts": args.cross_layout_layouts,
        "cross_layout_neighbors": args.cross_layout_neighbors,
        "proxy_decay": args.proxy_decay,
        "selection_mode": args.selection_mode,
        "lightsabre_total_swap": qiskit_total,
        "independent_rollout_total_swap": independent_rollout_total,
        "baseline_rollout_safe_total_swap": dual_total,
        "selected_total_swap": selected_total,
        "gain_vs_lightsabre": qiskit_total - selected_total,
        "gain_vs_baseline_rollout": dual_total - selected_total,
...
def main(
    *,
    default_cross_layout_layouts: int = 0,
    default_cross_layout_neighbors: int = 4,
) -> None:
    parser = argparse.ArgumentParser(
        description="SMPR live multi-layout portfolio router"
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        "--cross_layout-layouts",
        dest="cross_layout_layouts",
        type=int,
        default=default_cross_layout_layouts,
        help="最多使用多少个并列最优 LightSABRE 布局；0 表示全部",
    )
    parser.add_argument(
        "--cross-layout-neighbors",
        "--cross_layout-neighbors",
        dest="cross_layout_neighbors",
        type=int,
        default=default_cross_layout_neighbors,
    ...
    parser.add_argument(
        ...
        default="legacy",
        help=(
            "legacy reproduces frozen V5/V6 runtime tie-breaking; "
            "deterministic removes wall time from all quality ties"
        ),
    )
    ...
    if args.cross_layout_layouts < 0 or args.cross_layout_neighbors < 0:
        raise ValueError("cross_layout-layouts/neighbors 必须为非负整数")
    if not 0.0 < args.proxy_decay <= 1.0:
        raise ValueError("proxy-decay 必须位于 (0, 1]")
    global SELECTION_MODE
    SELECTION_MODE = args.selection_mode

    root = args.output_dir or Path("results/smpr_portfolio")
    ...
    print("===== SMPR live multi-layout portfolio router =====")
    ...
    print(
        "cross_layout:",
        f"best-quality layouts={args.cross_layout_layouts or 'all'}, "
        f"neighbors/layout={args.cross_layout_neighbors}, "
    ...
    for ordinal, (case_index, case) in enumerate(cases, start=1):
        ...
        independent_rollout_best, independent_rollout_trials, independent_rollout_variant = run_native_independent_rollout(case, config)
        baseline_rollout_safe = min((ls_best, independent_rollout_best), key=candidate_key)

        cross_layout_trials = run_cross_layout_candidates(
            case=case,
            qiskit_candidates=ls_trials,
            config=config,
            maximum_layouts=args.cross_layout_layouts,
            neighbors_per_layout=args.cross_layout_neighbors,
            proxy_decay=args.proxy_decay,
        )

        selectable = [ls_best, independent_rollout_best, *cross_layout_trials]
        selected = min(selectable, key=candidate_key)
        attempts = [*ls_trials, *independent_rollout_trials, *cross_layout_trials]
        attempt_rows.extend(
        ...
        cross_layout_best = (
            min(cross_layout_trials, key=candidate_key)
            if cross_layout_trials
            else None
        )
        ...
        row = {
            ...
            "independent_rollout_variant": independent_rollout_variant,
            "lightsabre_seed": ls_best.seed,
            "lightsabre_swap": ls_best.swap,
            "lightsabre_depth": ls_best.depth,
            "lightsabre_budget_ms": ls_budget_ms,
            "independent_rollout_swap": independent_rollout_best.swap,
            "independent_rollout_depth": independent_rollout_best.depth,
            "baseline_rollout_backend": baseline_rollout_safe.backend,
            "baseline_rollout_safe_swap": baseline_rollout_safe.swap,
            "baseline_rollout_safe_depth": baseline_rollout_safe.depth,
            "cross_layout_swap": "" if cross_layout_best is None else cross_layout_best.swap,
            "cross_layout_depth": "" if cross_layout_best is None else cross_layout_best.depth,
            "cross_layout_attempts": len(cross_layout_trials),
            "selected_backend": selected.backend,
            "selected_label": selected.label,
            "selected_swap": selected.swap,
            "selected_depth": selected.depth,
            "gain_vs_lightsabre": ls_best.swap - selected.swap,
            "gain_vs_baseline_rollout": baseline_rollout_safe.swap - selected.swap,
        ...
        summary = summary_payload(args.input, case_rows, attempt_rows, args)
        ...
        print(
            ...
            f"rollout={independent_rollout_best.swap}/{independent_rollout_best.depth}, "
            + (
                ""
                if cross_layout_best is None
                else f"cross-layout={cross_layout_best.swap}/{cross_layout_best.depth}, "
            )
            + (
                f"selected={PUBLIC_BRANCH.get(selected.backend, selected.backend)} "
                f"{selected.swap}/{selected.depth}, "
            )
    ...
    print("\n===== SMPR portfolio summary =====")
    print("cases:", summary["case_count"])
    print("LightSABRE total SWAP:", summary["lightsabre_total_swap"])
    print("independent rollout total SWAP:", summary["independent_rollout_total_swap"])
    print("baseline/rollout safe total SWAP:", summary["baseline_rollout_safe_total_swap"])
    print("SMPR total SWAP:", summary["selected_total_swap"])
    print("gain vs LightSABRE:", summary["gain_vs_lightsabre"])
    print("gain vs baseline/rollout safe:", summary["gain_vs_baseline_rollout"])
    print("selected branches:", summary["selected_branches"])
