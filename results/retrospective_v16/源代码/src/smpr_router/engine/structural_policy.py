...
# ordered_potential 的扩展前瞻衰减系数（V14 策略搜索可调；默认 0.5 = 原行为）。
ORDERED_DECAY = 0.5
...
def build_preview(
    ...
    extended: list[int] = []
    if (
        config.candidate_mode in ("ordered_frontier", "multigeodesic")
        or config.potential_mode
        in ("ordered_potential", "ordered_lin", "ordered_sq")
    ):
...
def generate_candidates(
    ...
    if config.candidate_mode == "multigeodesic":
        distances = ordered_candidates.distance_matrix(hardware)
        return ordered_candidates.multigeodesic_candidate_set(
            state=state,
            dag=dag,
            hardware=hardware,
            front=layers[0],
...
def ordered_linear_potential(
    preview: Preview,
    mapping: list[int],
    dag: CircuitDAG,
    hardware: HardwareGraph,
) -> float:
    """V16 新族：线性衰减 1/(1+index)，尾部比几何衰减（0.5^index）更重。"""
    if not preview.layers:
        return 0.0
    ...
    for index, gate_id in enumerate(preview.extended):
        value += (1.0 / (1.0 + index)) * one_step_policy.routing_distance(
            gate_id, mapping, dag, hardware
        )

    return value


def ordered_squared_potential(
    preview: Preview,
    mapping: list[int],
    dag: CircuitDAG,
    hardware: HardwareGraph,
) -> float:
    """V16 新族：平方路由距离，惩罚远距离门更狠。"""
    if not preview.layers:
        return 0.0

    value = sum(
        one_step_policy.routing_distance(gate_id, mapping, dag, hardware) ** 2
        for gate_id in preview.layers[0]
    )
...
def potential_value(
    ...
    if config.potential_mode == "ordered_lin":
        return ordered_linear_potential(preview, mapping, dag, hardware)

    if config.potential_mode == "ordered_sq":
        return ordered_squared_potential(preview, mapping, dag, hardware)
